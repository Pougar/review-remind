...
declare global {
  ...
  var _pgPoolUpdateBusinessSlug: Pool | undefined;
}
function getPool(): Pool {
  if (!global._pgPoolUpdateBusinessSlug) {
    const cs = process.env.DATABASE_URL;
    if (!cs) throw new Error("DATABASE_URL is not set");
    global._pgPoolUpdateBusinessSlug = new Pool({
      connectionString: cs,
      ssl: { rejectUnauthorized: false },
      max: 5,
    });
  }
  return global._pgPoolUpdateBusinessSlug;
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { businessId, newSlug } = (await req.json().catch(() => ({}))) as ReqBody;

    if (!isUUID(businessId)) {
      return NextResponse.json({ error: "MISSING_OR_INVALID_BUSINESS_ID" }, { status: 400 });
    }
    if (typeof newSlug !== "string") {
      return NextResponse.json({ error: "MISSING_NEWSLUG" }, { status: 400 });
    }
    ...
    // Check availability (case-insensitive by virtue of normalized lowercasing)
      ...
      `
        UPDATE public.businesses
           SET slug = $2,
               updated_at = NOW()
         WHERE id = $1
         RETURNING id, slug, display_name, company_logo_url, google_business_link, updated_at
      `,
    ...
    console.error("[update-business-slug] error:", (err as any)?.stack || err);
